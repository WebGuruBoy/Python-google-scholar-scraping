...
class URLFactory:
    ...
    def next_url(self, soup: BeautifulSoup = None) -> str:
        ...
        if self.type_ == ScholarURLType.CITATIONS and soup is not None:
            raw_next_url = soup.find('button', attrs={"aria-label": "Next"})
            if raw_next_url and raw_next_url.get('onclick'):
                raw_next_url = raw_next_url['onclick']
    ...
    # get next_url by page number
    def next_url_by_page(self, soup: BeautifulSoup = None, keyword = "", offset = 0) -> str:
        if self.url is None:
            raise UrlError()

        if self.type_ == ScholarURLType.CITATIONS and soup is not None:
            search_url = "/citations" \
                       "?view_op=search_authors&hl=en" \
                       "&mauthors={0}&after_author=DD-CAGhi_f8J" \
                       "&astart={1}".format(keyword, offset)
            print("next_url========{0}".format(ScholarURLType.BASE.value + search_url))
            return ScholarURLType.BASE.value + search_url
